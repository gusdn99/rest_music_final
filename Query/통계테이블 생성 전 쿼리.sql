SELECT 
    ...
    COUNT(al.album_id) AS like_count -- 20 좋아요 개수 COUNT로 계산
FROM albums a
LEFT JOIN album_likes al ON a.album_id = al.album_id -- 좋아요 테이블을 LEFT JOIN
WHERE a.album_name LIKE CONCAT('%', '20', '%')
...
WHERE g.group_name LIKE CONCAT('%', '20', '%')
...
WHERE art.artist_name LIKE CONCAT('%', '20', '%')
